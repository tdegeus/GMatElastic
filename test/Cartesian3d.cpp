...
SECTION("Elastic")
{
    ...
    double gamma = 0.02;
    double epsm = 0.12;
    auto Eps = GM::Tensor2::from_shape({3, 3});
    Eps.fill(0.0);
    ...
    ISCLOSE(Sig(0, 0), 3.0 * K * epsm);
    ISCLOSE(Sig(1, 1), 3.0 * K * epsm);
    ISCLOSE(Sig(2, 2), 3.0 * K * epsm);
    ISCLOSE(Sig(0, 1), 2.0 * G * gamma);
    ISCLOSE(Sig(1, 0), 2.0 * G * gamma);
    ISCLOSE(Sig(0, 2), 0.0);
    ISCLOSE(Sig(1, 2), 0.0);
    ISCLOSE(Sig(2, 0), 0.0);
    ISCLOSE(Sig(2, 1), 0.0);
...
SECTION("Array")
{
    ...
    for (size_t e = 0; e < nelem; ++e) {
        for (size_t q = 0; q < nip; ++q) {
            double fac = static_cast<double>((e + 1) * nip + (q + 1));
            ISCLOSE(sig(e, q, 0, 0), fac * 3.0 * K * epsm);
            ISCLOSE(sig(e, q, 1, 1), fac * 3.0 * K * epsm);
            ISCLOSE(sig(e, q, 2, 2), fac * 3.0 * K * epsm);
            ISCLOSE(sig(e, q, 0, 1), fac * 2.0 * G * gamma);
            ISCLOSE(sig(e, q, 1, 0), fac * 2.0 * G * gamma);
        }
    }
